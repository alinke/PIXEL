...
if [[ -d "$HOME/pixelcade" ]]; then
            while true; do
                read -p "${magenta}Existing Pixelcade installation detected. Do you want to check for updated files and artwork before continuing? (y/n) ${white}" yn
                case $yn in
                    [Yy]* ) cd $HOME/pixelcade/system && sh ./update.sh; break;;
                    [Nn]* ) break;;
